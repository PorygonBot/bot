...
function track(line: string, parts: string[], rules: { [key: string]: string }, battle: Battle, dataArr: string[]) {
	...
	//Ally Switch and stuff
		...
		console.log(
			`${battle.battlelink}: ${
				battle[`${userSide}a` as const].realName || battle[`${userSide}a` as const].name
			} has swapped with ${
				battle[`${userSide}b` as const].realName || battle[`${userSide}b` as const].name
	...
	//Moves that last for a single turn like Powder or Protect
		...
		let move = parts[2];
		let victimSide = parts[1].split(': ')[0] as 'p1a' | 'p1b' | 'p2a' | 'p2b';
		let prevMoveLine = dataArr[dataArr.length - 2];
		let prevMoveUserSide = prevMoveLine.split('|').slice(1)[1].split(': ')[0] as 'p1a' | 'p1b' | 'p2a' | 'p2b';
	...
	//For moves like Infestation and Fire Spin
		...
		if (
			...
			let victimSide = parts[1].split(': ')[0] as 'p1a' | 'p1b' | 'p2a' | 'p2b';
			let inflictorSide = parts[3].split(' ')[1].split(':')[0] as 'p1a' | 'p1b' | 'p2a' | 'p2b';
	...
	//Checks for certain specific moves: hazards only for now
		...
		if (line.includes('[miss]')) {
			//If a mon missed
			let inflictorSide = parts[1].split(': ')[0] as 'p1a' | 'p1b' | 'p2a' | 'p2b';
			let victimSide = parts[3].split(': ')[0] as 'p1a' | 'p1b' | 'p2a' | 'p2b';
			battle.history.push(
	...
	//Critical hit
		...
		let inflictorSide = prevParts[1].split(': ')[0] as 'p1a' | 'p1b' | 'p2a' | 'p2b';
	...
	//Statuses
		...
		if (prevMoveLine.includes('Synchronize')) {
			...
			(prevPrevMoveLine.startsWith(`|move|`) && consts.statusMoves.includes(prevPrevMove))
		) {
			...
			(line.includes('ability') && consts.statusAbility.includes(parts[3].split('ability: ')[1].split('|')[0])) ||
			line.includes('item')
		) {
			//Ability status
			let inflictorSide = (line.includes('item: ') ? victimSide : parts[4].split('[of] ')[1].split(': ')[0]) as
				| 'p1a'
				| 'p1b'
				| 'p2a'
				| 'p2b';
	...
	//If a hazard ends on a side
		...
		let removerSide = (parts[4] ? parts[4].split('[of] ')[1].split(': ')[0] : prevMoveParts[1].split(': ')[0]) as
			| 'p1a'
			| 'p1b'
			| 'p2a'
			| 'p2b';
	...
	//If an affliction like Leech Seed or confusion starts
		...
		if (
			...
			(prevMove.split('|').slice(1)[2] === affliction.split('move: ')[1] ||
				...
				consts.confusionMoves.includes(prevMove.split('|').slice(1)[2]) || //For confusion
			...
			let afflictorSide = prevMove.split('|').slice(1)[1].split(': ')[0] as 'p1a' | 'p1b' | 'p2a' | 'p2b';
			let afflictor;
		...
		if (affliction === `perish0`) {
			...
			if (battle[`${currentPlayer}Pokemon` as const][afflictor] && afflictor !== victim) {
	...
	//If a pokemon's status is cured
		...
		if (!(side.endsWith('a') || side.endsWith('b'))) {
			for (let pokemon of Object.keys(battle[`${playerSide}Pokemon` as const])) {
	...
	//When a Pokemon is damaged, and possibly faints
		...
		if (parts[2].endsWith('fnt') || parts[2].startsWith('0')) {
			...
			let oppositeSide = (
				victimSide.startsWith('p1') ? victimSide.replace('1', '2') : victimSide.replace('2', '1')
			) as 'p1a' | 'p1b' | 'p2a' | 'p2b';
			...
			//If an affliction triggered
				...
				if (
					...
					let prevMoveUserSide = prevMoveParts[1].split(': ')[0] as 'p1a' | 'p1b' | 'p2a' | 'p2b';
	...
	//This is mostly only used for the victim of Destiny Bond
		...
		if (prevLine.startsWith(`|-activate|`) && prevLine.endsWith(`Destiny Bond`)) {
			...
			let victim = '';
			if (!battle[victimSide].isDead) {
				let killerSide = prevParts[1].split(': ')[0] as 'p1a' | 'p1b' | 'p2a' | 'p2b';
